  ...
  <div>
    <div class="container">
        <div class="d-flex justify-content-between">
          <div>
            <div class="tombol-modify">Detail Riwayat</div>
          </div>
          <div class="float-right tom">
            <router-link to="/user" title="Kembali" class="btn btn-sm btn-dark ml-1"><i class="fas fa-arrow-left"></i></router-link>
          </div>
        </div>
      <div class="card">
        <div class="card-body">
                ...
                <tr>
                  ...
                  <td>
                    <div v-if="denda(transaksis.tgl_pengembalian) > 0" style="color:red;font-weight:bold;">
                      <div class="float-left">{{ denda(transaksis.tgl_pengembalian) }} hari |</div> <FormatRupiah :angka="denda(transaksis.tgl_pengembalian) * 1000 * transaksis.jumlah" :prefix="'Rp. '" class="ml-1 float-left"/>
                    </div>
                    <div v-else:>---</div>
                  </td>
                </tr>
  ...
  setup(){
    ...
    const getTransaksi = async () => {
      try{
        ...
        let response = await axios.get('api/user/detailRiwayat/' + par.params.kode)
        if(response.status === 200){
          transaksis.value = response.data.transaksi
          peminjaman.value = response.data.pinjam
          loader.hide()
        }    
      }catch{
        router.replace('/user')
